...
function Home(props) {
  ...
  <a className="carousel-control-next" href="#carouselExampleControls" role="button" data-slide="next">
    ...
    {arr.map((element)=>{return element.attributes.Download.data?<a style={{color:'black',textDecoretion:'none'}} href={`${process.env.REACT_APP_SERVERNAME}${element.attributes.Download.data[0].attributes.url}`}><div key={element.id} style={{display:'flex',flexDirection:'row',alignItems:'center',marginTop:'15px'}}><img src={Img2} alt='new'/><div style={{width:'70%'}}><div style={{marginBottom:'0px',display:'flex',justifyContent:'space-between'}}>{element.attributes.Heading?<div>{element.attributes.Heading}</div>:<div>...</div>}<div style={{fontSize:'12px'}}>{formatMyDate(element.attributes.createdAt)}</div></div><div style={{fontSize:'12px'}}>{element.attributes.Description?<div>{element.attributes.Description.slice(0,25)}...</div>:<div>...</div>}</div></div></div></a>:<div key={element.id} style={{display:'flex',flexDirection:'row',alignItems:'center',marginTop:'15px'}}><img src={Img2} alt='new'/><div style={{width:'70%'}}><div style={{marginBottom:'0px',display:'flex',justifyContent:'space-between'}}><div>{element.attributes.Heading?<div>{element.attributes.Heading}</div>:<div>...</div>}</div><div style={{fontSize:'12px'}}>{formatMyDate(element.attributes.createdAt)}</div></div><div style={{fontSize:'12px'}}>{element.attributes.Description?<div>{element.attributes.Description.slice(0,25)}...</div>:<div>...</div>}</div></div></div>})}
...
Co-operation, Jal Shakti, Transport, Language, Arts and Culture Minister</p>
      ...
      <div className='personality'><div className='personality-image'><img src={Person3} alt='personality'/></div><div style={{margin:'20px 20px',height:'120px'}}><b style={{fontSize:'18px'}}>Sh.Sandeep Kadam, IAS</b><br/><p style={{fontSize:'14px'}}>Registrar Cooperative Societies</p></div></div>
    </div>
  </div>
...
forefront of cooperative education and training in Himachal Pradesh....<br/><br/>
<Link style={{textDecoration:'none',width:'100%',display:'flex',justifyContent:'center',alignItems:'center'}} onClick={()=>{window.scroll(0,0)}} to='/admin'><button style={{width:'50%',borderRadius:'30px',backgroundColor:"white",border:'2px solid black',color:'rgb(184, 0, 0)',boxShadow:'1px 1px 10px 1px black'}}>Read More</button></Link>
